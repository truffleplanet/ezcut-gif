...
6. 예전에 작업했던 내역은 언제든 `History` 탭을 열고 항목을 더블클릭해서 다시 Preview 혹은 Upload 탭으로 불러올 수 있습니다.
...
`History` 탭은 이전에 작업한 내역을 전체적으로 조회하고 다시 빠르게 불러오기 위해 사용됩니다.

### 주요 기능

- **히스토리 내역 조회**
  - 이름, 조각 수, 일시 외에도 원본 GIF에서 Mattermost에 `업로드`했는지, 갤러리에 `공유`했는지(🔗 마크 등) 여부를 표 형식으로 간편하게 볼 수 있습니다.

- **자동 불러오기 (Load)**
  - 특정 항목을 마우스로 더블 클릭하거나 하단의 `이 항목 불러오기` 버튼을 누르면, 해당 결과 폴더의 경로가 `Preview` 탭과 `Upload` 탭의 `타일 디렉토리` 입력 칸에 자동으로 바인딩(동기화)됩니다.
  - 불러오기가 완료되면 툴바에서 시각적 확인 메시지를 띄운 뒤 즉시 원활한 작업 흐름을 위해 `Upload` 탭 화면으로 자동 이동합니다.

### 언제 사용하면 좋은가

- 며칠 전에 자른 GIF 조각을 다시 갤러리에 투고하거나, 업로드에 오류가 나서 재시도하고 싶을 때
- 과거 업로드 상태와 공유 상태를 눈으로 한 번에 확인하고 싶을 때
...
- `History`는 탭은 예전 결과 폴더 작업을 한 번에 확인하고 불러오는 편의 탭
